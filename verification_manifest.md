...
| C53 | Dominant-regime stability across the FULL 23-market panel (superseded the original 17-named-market version, which raised its own question of why 6 markets were excluded from the check - see the note below) | `scripts/group_membership_stability.py`: **22 of 23 panel markets** keep the same dominant regime (RW for every market but one, EXP for Japan) across all six min-regime rules plus a fresh constant-drift (spec=2) refit - 7/7 agreement. The one exception is Japan: EXP stays dominant under only 3/7 alt specs (flips to RW-dominant under pct10/pct15/obs30/obs40), consistent with Japan's already-documented degenerate-threshold caveat (`config.EXCLUDED_EXP_MARKETS`) rather than a new problem. Panel-wide rather than group-restricted specifically so the result doesn't depend on which markets Section 4.2's prose happened to name - one number, one panel, nothing to explain about scope. Does NOT test the finer boundaries between the four heuristic groups (episode-timing evidence, not regime shares) - see the script's docstring caveat. | E | P3 |
| C54 | Positive control: block placebo on the two ORIGINAL Ahmed & Satchell (2018) pairings (S&P500+MCSI, FTSE100+VIX), not the 23-market panel | `scripts/placebo_original_pairs.py`. Seed `config.SEED_PLACEBO_T1` (20260721), B=`config.B_PLACEBO` (1000), identical `rss_single`/`tar_fit`/`improvement` functions as the main-panel placebo (`placebo_1000.py`), `src/replicate.py`'s own data/grid spec (not the panel's adaptive `grid_bounds`). Both pairings clear the block12 null: S&P500+MCSI real=7.953%, block12 p50/p95=2.983/4.807 (iid) discarded - block12 p50=3.147 p95=5.829, emp_p=0.0060, 99.4th pctile; FTSE100+VIX real=2.498%, block12 p50=0.243 p95=0.525, emp_p=0.0000, 100th pctile. This is the positive control for the main panel's 3-of-23 result (A5): the estimator identifies a genuine split when the trigger actually matches the market it was built for, so 3-of-23 reads as a transportability/scope finding, not an estimator defect. Belongs in Section 4.2 next to A5. | S | P1 |
| C55 | S&P500 under the Global OECD CCI (not MCSI) - secondary/diagnostic only | `scripts/placebo_sp500_global.py`, same seed/B/design as C54. real=5.338% (c1=97.67, c2=101.93), block12 p50=3.029 p95=6.456, emp_p=**0.0940** - fails to clear p<0.05. sp500 fails the reverse-Granger screen (US equity returns move the GDP-weighted global aggregate), so endogeneity biases this test toward passing; only the failure is interpretable, a pass would tell us nothing. Must carry this caveat explicitly wherever cited (Section 4.2 or 5.1). | S | P2 |
| C56 | Within-market trigger-swap placebo: same market, same matched-window sample, only the trigger changes (global CCI vs national CCI) | `scripts/placebo_trigger_swap.py`, same seed/B/design as C54. ATHEX (T=317): global block12 p=0.064, Greek CCI block12 p=0.072 - essentially identical; the national trigger does NOT identify better, consistent with Section 5's actual claim (where months land, not statistical identification strength). BIST100 (T=240): global p=0.074, Turkish CCI p=0.254 - national notably worse, consistent with Turkey being a documented false positive. Shanghai (T=317): global p=0.361, Chinese CCI (live FRED fetch, see B4) p=0.820 - both fail, national worse. SZSE (T=316): global p=0.544, Chinese CCI p=0.761 - both fail, national worse. Do not claim the national trigger identifies better anywhere in the manuscript - only ATHEX's near-parity and Turkey/China's national-worse results are supported. Belongs in Section 5. | S | P1 |
| C57 | Window-length effect (isolated from trigger choice) and RSS-improvement/T rank correlations vs the placebo p-value | `scripts/placebo_window_and_rank.py`. Window effect: ATHEX global block12 p goes from 0.004 (T=345, full sample) to 0.064 (T=317, matched window) - same trigger, same market, only T changes; BIST100 global goes from 0.025 (T=346) to 0.074 (T=240). Both markets' full-sample "passing" result is not robust to using less history, independent of trigger choice - state this alongside C56, do not let a reader attribute the full-sample-vs-matched-window difference to the trigger swap alone. Rank correlations (n=23, block12 null): Spearman(real_rss_impr, emp_p_rss) = -0.874 (p=5.3e-08) - which markets clear the placebo is explained substantially by realised RSS improvement (Brazil 28.2%, next tier 9.98%). Spearman(T, emp_p_rss) = 0.080 (p=0.715) - essentially zero; sample length does NOT predict passing within the panel's own T range (345-436, no admissible market below 345). No contradiction with the window-effect finding above: that involved cutting T down to 317/240, a range that does not otherwise occur in the panel - within the panel's actual range, length is unrelated to outcome. State both correlations together: RSS improvement explains the pattern, sample length does not. Suggested line for Section 4.2, next to A1a's passer list. | S | P2 |
| C58 | Ex-COVID sub-sample: does excluding 2020-01 to 2021-12 move the pooled 12m forward means by band | `scripts/ex_covid_subsample.py`, reuses `horizon_episodes.load_all()`/`pooled()` directly. 552 index-months fall in the excluded window across all markets, of which 101 are classified low-sentiment (9.1% of the 1109-obs MR cell). Excluding them moves the MR mean from 14.601 to 14.921 (n 1109->1008) - UP, not down, refuting the worry that COVID inflated it. RW mean 8.009->7.854 (n 6676->6233). High-band mean exactly unchanged (n=615 both ways) - zero EXP-classified months fall in this window. The concern does not materialise. | E | P2 |
| C59 | Passer composition: which 3 of 23 panel markets clear the block12 null, with RW% and T, tested against a market-depth/liquidity explanation | Lookup against `results/exports/placebo_1000.csv` (block12) and `results/tables/global_cci_all_markets.csv`. Passers: Greece (emp_p=0.0040, RW%=88.4, T=345), Turkey (emp_p=0.0250, RW%=94.2, T=346), Brazil (emp_p=0.0260, RW%=50.4, T=397). Hong Kong, Singapore and Switzerland (the deep-liquid-market hypothesis) sit mid-to-bottom of the ranking (emp_p 0.135/0.437/0.068) and do not clear it - the market-depth explanation for the 3-of-23 result is refuted directly by this lookup, not just argued against. See C57 for what does explain the ordering (RSS improvement, not T or market characteristics). | S | P2 |
| C60 | Price-blind version of the middle-band drift test (A4/C20): bands defined by each market's OWN 10th/90th trigger percentile, no reference to price fit, vs the RSS-fit RW band | `scripts/rw_band_validation_blind.py`. A4/C20's "unrestricted slope" check reuses the SAME (c1,c2) the RSS search already chose, and that search forces beta=0 for the RW state at every candidate (`src/estimate.py:129,156`) - so the band is partly optimized toward flatness before the "unrestricted" re-test runs. This version removes that: per-market percentile cutoffs (blind to price), same free (alpha,beta) OLS and BH correction as A4/C20, directly comparable. Result: 8/23 raw-reject (vs A4's 6/23), 1/23 survives BH (vs A4's 0/23) - MORE evidence of drift under the unbiased design, consistent with the circular test having been conservative as theorised. The one BH-surviving market is Bovespa (p_mid<0.0001, b_mid=-0.0371) - notable since Bovespa's ORIGINAL RSS-fit-band p_RW was 0.0643, not even raw-significant; its RW-band as chosen by the RSS search evidently absorbed real drift the price-blind band does not. Bovespa is also the single largest passer of the block12 placebo (C59, real_rss_impr=28.2%) - a consistent picture of one market with genuine structure the fit-dependent tests undercount. 22 of 23 markets still show no BH-significant drift, so the random-walk assumption is still broadly supported, but "comes back the same way" is not quite accurate - report both numbers and name Bovespa explicitly, do not round this to "unchanged." | E | P2 |
...
| 6.3 | Episode-level excess returns, 30 spells | RESOLVED: original generating script did not exist in this repo (embedded image's own metadata names it `fig6_3_episodes.png`, which appears nowhere in this codebase - a lost-script gap, though the data itself was already verified correct). Rebuilt as `scripts/fig6_3_episode_excess.py` from `episodes_exp.csv`; reproduces the title's t=-3.46/p=0.0017 exactly (asserted in-script) and matches the embedded chart's bar order/values/coloring. Output `outputs/rebuilt/fig6_3_episode_excess.png`. | P2 |
