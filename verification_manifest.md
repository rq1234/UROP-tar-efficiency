...
| C57 | Window-length effect (isolated from trigger choice) and RSS-improvement/p-value rank correlation | `scripts/placebo_window_and_rank.py`. Window effect: ATHEX global block12 p goes from 0.004 (T=345, full sample) to 0.064 (T=317, matched window) - same trigger, same market, only T changes; BIST100 global goes from 0.025 (T=346) to 0.074 (T=240). Both markets' full-sample "passing" result is not robust to using less history, independent of trigger choice - state this alongside C56, do not let a reader attribute the full-sample-vs-matched-window difference to the trigger swap alone. Rank correlation: Spearman(real_rss_impr, emp_p_rss) = -0.874 (p=5.3e-08, n=23, block12 null) - the ordering of which of the 23 panel markets clear the placebo is explained substantially by which had the largest realised RSS improvement (Brazil 28.2%, next tier 9.98%), not by market identity/characteristics. Suggested line for Section 4.2, next to A1a's passer list. | S | P2 |
