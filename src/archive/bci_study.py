"""
OECD Global Composite BCI as a universal trigger for the TAR bubble model.

OECD series BCICP (Composite Business Confidence, Amplitude-Adjusted):
composite Business Confidence across all OECD members, normalised to
100 = long-run average.  Above 100 = optimistic; below 100 = pessimistic.

SIGN CONVENTION: No negation.  High BCI -> EXP (euphoria/bubble risk).
                              Low  BCI -> MR  (pessimism/crisis).

BCI differs from CCI in that it measures business investment intentions and
production plans rather than consumer sentiment.  Because it is less
directly driven by the stock market wealth effect, it may be more exogenous
to equity returns than CCI for large markets (S&P 500, FTSE 100).

Note: BCI peaked at ~100.9 during 1999-2001 (below the 101 level often seen
for CCI), so the dot-com bubble is less clearly captured. GFC and COVID
troughs are sharp (-5 points below 100).

Six stages:
  1 - inspect   : plot global BCI with episode shading, summary stats
...
Usage:
    cd src && python bci_study.py              # full pipeline
    python bci_study.py inspect
    python bci_study.py screen
    python bci_study.py validate
    python bci_study.py rolling
    python bci_study.py estimate
    python bci_study.py bubble
"""
...
BCI_PATH = os.path.join(os.path.dirname(__file__), "..", "..", "data", "sentiment",
                         "bci_monthly.csv")
START = "1990-01"
...
def load_bci_monthly():
    """Return the monthly global OECD BCI series (100 = long-run average)."""
    s = pd.read_csv(BCI_PATH, index_col="date",
                    parse_dates=True)["bci"].dropna()
    s.index = s.index + pd.offsets.MonthEnd(0)
    return s


def load_bci_pair(market, start, end):
    """
    Load (y, z, dates) for a (market, global-BCI) pair.

    z = global OECD BCI level (no negation -- high = optimistic = EXP).
    """
    panel  = pd.read_csv(PANEL_PATH, index_col="date", parse_dates=True)
    prices = panel[f"eq_{market}"].dropna()

    bci  = load_bci_monthly()
    df   = pd.DataFrame({"price": prices, "bci": bci}).dropna().loc[start:end]

    y     = np.log(df["price"].values).astype(float)
    z     = df["bci"].values.astype(float)
...
def _grid_bounds(z):
    """
    Adaptive grid bounds for global BCI.
    Uses +/-3 sigma from mean.
    min_gap enforces a minimum band of 2.0 BCI points (same as CCI).
    """
...
def inspect_bci():
    """Plot global OECD BCI 1990-2026 with episode shading."""
    bci = load_bci_monthly().loc[START:]

    print(f"\nGlobal OECD BCI stats (1990-present): "
          f"mean={bci.mean():.3f}  std={bci.std():.3f}  "
          f"min={bci.min():.3f}  max={bci.max():.3f}")
    print(f"  Current (latest): {bci.iloc[-1]:.3f}  "
          f"as of {bci.index[-1].strftime('%Y-%m')}")
    print(f"  Note: BCI range is narrower than CCI; dot-com peak ~100.9 "
          f"(vs CCI ~101.5), GFC trough ~95.3.")

    os.makedirs(FIGURES_DIR, exist_ok=True)
    fig, ax = plt.subplots(figsize=(13, 4))

    dates_np = np.array(bci.index, dtype="datetime64[ns]")
    ax.plot(dates_np, bci.values, color="black", linewidth=0.8)
    ax.axhline(100, color="gray", linewidth=0.7, linestyle="--",
               alpha=0.7, label="100 (long-run average)")
    ax.fill_between(dates_np, bci.values, 100,
                    where=(bci.values > 100), color="tomato",    alpha=0.20)
    ax.fill_between(dates_np, bci.values, 100,
                    where=(bci.values < 100), color="cornflowerblue", alpha=0.20)

    for label, start, end, color in EPISODES:
        ep = bci.loc[start:end]
        if len(ep):
    ...
    ax.set_title("OECD Global Composite BCI (BCICP)  "
                 "-- above 100 = optimistic / bubble risk", fontsize=9)
    ax.set_ylabel("BCI (100 = long-run avg)", fontsize=8)
    ax.tick_params(labelsize=7)
    ax.margins(x=0.01)

    plt.tight_layout()
    out = os.path.join(FIGURES_DIR, "bci_overview.png")
...
def exogeneity_screen_bci(n_lags=4, alpha=0.05):
    """
    Reverse Granger test: does equity return Granger-cause delta(global BCI)?
    If yes, the global BCI reflects equity sentiment -> endogenous -> FAIL.
    Gate: p > alpha -> ADMISSIBLE.

    BCI is based on business surveys (production plans, orders) rather than
    consumer sentiment, so it should be less driven by equity wealth effects.
    Expect more markets to pass than with CCI.
    """
    print(f"\n{'='*70}")
    print("STAGE 2 -- Exogeneity screen (reverse Granger: return -> d(global BCI))")
    print(f"{'='*70}")
    ...
    for market, info in MARKETS.items():
        try:
            y, z, _ = load_bci_pair(market, START, FULL_END)
            y_ret  = np.diff(y)
            z_diff = np.diff(z)
...
def _episode_verdict(state, dates, min_months=2, verbose=True):
    ...
    EXP (z > c2): high global BCI -> optimism / bubble risk
    MR  (z < c1): low  global BCI -> pessimism / crisis
    """
...
def validate_us_bci():
    """S&P 500 + global BCI: full-sample estimation and episode overlap."""
    print(f"\n{'='*70}")
    print("STAGE 3 -- Validation: S&P 500 + global BCI (full sample 1990-2026)")
    print(f"{'='*70}")

    y, z, dates = load_bci_pair("sp500", START, FULL_END)
    z_min, z_max, gl, mg = _grid_bounds(z)
    print(f"  T={len(y)}  BCI=[{z.min():.3f}, {z.max():.3f}]  "
...
def rolling_bci_stability():
    """10-year rolling-window thresholds for S&P 500 + global BCI."""
    print(f"\n{'='*70}")
    print("STAGE 4 -- Rolling-window stability (S&P 500 + global BCI)")
    print(f"{'='*70}")

    y, z, dates = load_bci_pair("sp500", START, FULL_END)
    z_min, z_max, gl, mg = _grid_bounds(z)

    y_in, z_in, _ = load_bci_pair("sp500", START, INSAMPLE_END)
    ...
    csv_path = os.path.join(TABLES_DIR, "rolling_thresholds_SP500_BCI.csv")
    ...
    ax.set_title("SP500 + global BCI -- rolling 10-yr TAR thresholds  "
                 "[100 = long-run avg]", fontsize=9)
    ax.set_xlabel("Window end date", fontsize=8)
    ax.set_ylabel("BCI threshold", fontsize=8)
    ax.legend(fontsize=7, ncol=2)
    ax.tick_params(labelsize=7)
    ax.margins(x=0.01)
    plt.tight_layout()
    out = os.path.join(FIGURES_DIR, "rolling_thresholds_SP500_BCI.png")
    plt.savefig(out, dpi=150)
    plt.close()
    print(f"  Saved: {out}")
...
def _estimate_one(market, start=START, end=FULL_END, spec=1, verbose=True):
    """TAR estimation + episode verdict for one market with global BCI trigger."""
    y, z, dates = load_bci_pair(market, start, end)
    if len(y) < 120:
        return None
...
def run_all_markets(admissible=None):
    """Stage 5: TAR for all (or admissible) markets with global BCI trigger."""
    print(f"\n{'='*70}")
    print("STAGE 5 -- Cross-market estimation (global BCI trigger, spec=1)")
    print(f"{'='*70}")
    ...
    pd.DataFrame(rows).to_csv(
        os.path.join(TABLES_DIR, "bci_all_markets.csv"), index=False
    )
    print(f"\nSaved: {os.path.join(TABLES_DIR, 'bci_all_markets.csv')}")
    return rows
...
def bubble_call(admissible=None):
    ...
    """
    print(f"\n{'='*70}")
    print("STAGE 6 -- Bubble call (Option A + B, global BCI trigger)")
    print(f"{'='*70}")
    ...
    for market in markets:
        label = f"{market.upper()}_BCI"
        try:
            y, z, dates = load_bci_pair(market, START, FULL_END)
            if len(y) < 120:
                continue
            z_min, z_max, gl, mg = _grid_bounds(z)
            ...
            y_in, z_in, _ = load_bci_pair(market, START, INSAMPLE_END)
...
def run_all():
    inspect_bci()
    admissible = exogeneity_screen_bci()
    validate_us_bci()
    rolling_bci_stability()
...
if __name__ == "__main__":
    ...
    dispatch = {
        "inspect":  inspect_bci,
        "screen":   exogeneity_screen_bci,
        "validate": validate_us_bci,
        "rolling":  rolling_bci_stability,
