"""
Parse the official OECD Global Composite CCI from the SDMX CSV download.

Source file: data/sentiment/global_oecd_cci.csv
  Downloaded from OECD.Stat: MEI_CLI dataset, REF_AREA=OECD, MEASURE=CCICP,
  ADJUSTMENT=AA (amplitude-adjusted, long-run average = 100).

Relevant columns in the SDMX export:
  TIME_PERIOD : YYYY-MM date string
  OBS_VALUE   : CCI value (100 = long-run average)

Output: data/sentiment/global_cci_monthly.csv  (date, global_cci)

Usage:
    cd src && python collect_global_cci.py
"""
...
SOURCE_PATH      = os.path.join(SENTIMENT_DIR, "global_oecd_cci.csv")
GLOBAL_CCI_PATH  = os.path.join(SENTIMENT_DIR, "global_cci_monthly.csv")


def collect_global_cci():
    if not os.path.exists(SOURCE_PATH):
        raise FileNotFoundError(
            f"Source file not found: {SOURCE_PATH}\n"
            "Download from OECD.Stat: MEI_CLI, REF_AREA=OECD, MEASURE=CCICP, AA."
        )
    ...
    print(f"Official OECD Global CCI: {len(s)} monthly obs  "
    ...
    s.to_frame().to_csv(GLOBAL_CCI_PATH)
    print(f"Saved: {os.path.abspath(GLOBAL_CCI_PATH)}")
