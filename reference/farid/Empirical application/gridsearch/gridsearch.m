...
function [stats] = gridsearch(y,z,c1,c2,intercept)
%We define our grid in order to first split the sample into 3

%in this program state 1 is efficient, state 2 is mean reverting and state
%3 is explosive
count = zeros(3,1);
length = prod(size(y));
...
%separately for each. 
for i = 2:length
    if z(i)<c1 %in this step we identify the 1st or mean reverting state
        count(2,1)=count(2,1)+1;
        meanrevert(i,1) = 1;
    elseif z(i) > c2 %in this step we identify the explosive state
        count(3,1) = count(3,1)+1;
        explosive(i,1) = 1; 
    else
        count(1,1) = count(1,1)+1; %the remaining entries are considered efficient
    end
end
...
y1 = zeros(count(1,1),1); x1 = zeros(count(1,1),1); %this variable will contain the efficient sub-sample with x's being the lagged values
y2 = zeros(count(2,1),1); x2 = zeros(count(2,1),1); %this variable will contain the mean reverting sub-sample with x's being the lagged values
y3 = zeros(count(3,1),1); x3 = zeros(count(3,1),1); %this variable will contain the explosive sub-sample with x's being the lagged values
k1 = 0; k2 = 0; k3=0;

%Now we will the split samples with actual values

for j = 2:length
    if z(j)<c1 %this steps fills out the mean reverting variable
        k2 = k2+1;
        y2(k2) = y(j,1);
        x2(k2) = y(j-1,1);
    elseif z(j)>c2 %this step fills out the explosive variable
        k3 = k3+1;
        y3(k3) = y(j,1);
        x3(k3) = y(j-1,1);
    else
        k1 = k1+1; %this step fills out the efficient variable
        y1(k1) = y(j,1);
        x1(k1) = y(j-1,1);
    end
end
...
if intercept == 1 %this model gives us a switching alpha
    mdl2 =fitlm(x2,y2,'Intercept',true);
    mdl2.Coefficients;
    mdl3 =fitlm(x3,y3,'Intercept',true);
    mdl3.Coefficients;
    const = ones(count(1,1),1);
    mdl1 = fitlm(const,y1-x1,'Intercept',false);
    stats.beta1 = 1;    
    ...
    residuals = mdl1.Residuals.Raw + mdl2.Residuals.Raw + mdl3.Reisduals.Raw;
elseif intercept == 2 %this model gives us a single alpha
    ...
    stats.alpha3 = 0;
    stats.beta1 = 1;
    stats.beta2 = 1+ mdl.Coefficients.Estimate(2,1);
    stats.beta3 = 1 + mdl.Coefficients.Estimate(3,1);
    mdl.SSR;
    %mdl = fitlm(tbl,'y~x+meanrevert*x+explosive*x')
    residuals = mdl.Residuals.Raw;
else %this model has alpha = 0
...
if intercept == 2
    ...
    for p = 2:prod(size(y))
        if z(p) < c1
            e(p) = y(p) - stats.beta2*y(p-1)-stats.alpha1;
        elseif z(p) > c2
            e(p) = y(p) - stats.beta3*y(p-1)-stats.alpha1;
        else
            e(p) = y(p) - stats.beta1*y(p-1)-stats.alpha1;
        end
    end
    stats.RSS = sum(e.^2); %this finds the sum of squared residuals
    ...
    % test;
    for p = 1:count(1,1)
     e1(p) = y1(p) - stats.alpha1 - stats.beta1*x1(p);
    end
    ...
    for p = 2:prod(size(y))
         if z(p) < c1
             e(p) = y(p) - stats.beta2*y(p-1) - stats.alpha2;
        elseif z(p) > c2
             e(p) = y(p) - stats.beta3*y(p-1) - stats.alpha3;
         else
             e(p) = y(p) - stats.beta1*y(p-1)-stats.alpha1;
         end
    end
    ...
    RSS2 ;
    RSS3;
    %stats.RSS = RSS1 + RSS2 + RSS3; %This calculates the total sum of square residuals
    stats.RSS = test2;
