function [parameters] = switchingparam(c1,y,z)
...
for i = 1:(prod(size(range))-1) %this part of the loop sets the value of the mean reverting parameter
    ...
    for k = j:(prod(size(range))) %the second loop ensures that c2 < c3 thereby reducing the number of loops
        ...
        for l = 2:length
            if z(l)<70 %criteria for mean reversion
             beta = c2;
            elseif z(l)>99 %criteria for explosivity
             beta = c3;
            else
             beta = c1; %random walk
            end
         e(l,1) = y(l)-beta*y(l-1); %this step calculates the error term
        end
